...
export const Switch = ({
  ...
  const { paperTheme: theme } = usePaperTheme();
  ...
  const trackStyle = useAnimatedStyle(() =>
    disabled
      ? {
          ...
          width: 52,
          opacity: 0.12,
          backgroundColor: active ? theme.colors.onSurface : theme.colors.surfaceVariant,
  ...
  const handleOutlineStyle = useAnimatedStyle(() => ({
    ...
    backgroundColor: !isPressed
      ? 'transparent'
      : interpolateColor(position.value, [-10, 10], [theme.colors.onSurface, theme.colors.primary]),
    alignItems: 'center',
    opacity: 0.18,
    justifyContent: 'center',
  }));
  ...
  const iconOnStyle = useAnimatedStyle(() => ({
    opacity: disabled ? 0.38 : interpolate(position.value, [0, 10], [0, 1], Extrapolation.CLAMP),
    overflow: 'hidden',
    transform: [
      {
        scale: interpolate(position.value, [0, 10], [0, 1], Extrapolation.CLAMP),
      },
      {
        rotate: `${interpolate(position.value, [0, 10], [-90, 0], Extrapolation.CLAMP)}deg`,
      },
    ],
  }));
  const iconOffStyle = useAnimatedStyle(() => ({
    position: 'absolute',
    opacity: disabled ? 0.38 : interpolate(position.value, [-10, 0], [1, 0], Extrapolation.CLAMP),
    overflow: 'hidden',
    transform: [
      {
        scale: interpolate(position.value, [-10, 0], [1, 0], Extrapolation.CLAMP),
      },
      {
        rotate: `${interpolate(position.value, [-10, 0], [-90, 0], Extrapolation.CLAMP)}deg`,
      },
    ],
          ...
          {switchOnIcon ? (
            <Animated.View key={10} style={iconOnStyle}>
              <Icon
                source={switchOnIcon}
                size={16}
                color={disabled ? theme.colors.onSurface : theme.colors.onPrimaryContainer}
              />
            </Animated.View>
          ) : null}
          {switchOffIcon ? (
            <Animated.View key={9} style={iconOffStyle}>
              <Icon source={switchOffIcon} size={16} color={theme.colors.surface} />
            </Animated.View>
          ) : null}
        </Animated.View>
      </View>
